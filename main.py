...
def get(conn, url):
	try:
		...
		if res.status == 200:
			...
			if h != hostname:
				logging.warn("hostname change: %s", h)
				return
			if loc == url:
				logging.warn("url loop: %s", loc)
				return
			return get(conn, loc)
		elif res.status > 200 and res.status < 300:
			logging.warn("%s 2xx status", url)
		else:
			logging.warn("%s unknown status", url)
	finally:
		conn.close()
...
class ThreadItem:
	def __init__(self, id, title, catalog='Unknown', author='', create_time='', last_reply_time='', reply_num=-1, desc=''):
		self.id = id
		self.title = title
	...
	for s in thread_list:
		try:
			...
			if s.find(text="贴吧游戏"):
				...
				logging.warn("broken thread: %s", s)
			continue
	return l
